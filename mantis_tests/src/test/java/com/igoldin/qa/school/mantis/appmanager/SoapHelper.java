...
public class SoapHelper {
    ...
    public Set<ProjectData> getProjects() throws MalformedURLException, ServiceException, RemoteException {
        ...
        biz.futureware.mantis.rpc.soap.client.ProjectData[] projects = mc.mc_projects_get_user_accessible("administrator", "root");
        return Arrays.asList(projects).stream()
                .map((p) -> new ProjectData().withId(p.getId().intValue()).withName(p.getName())).collect(Collectors.toSet());
    }

    public MantisConnectPortType getMantisConnect() throws ServiceException, MalformedURLException {
        return new MantisConnectLocator().getMantisConnectPort(new URL("http://localhost/mantis/api/soap/mantisconnect.php"));
    }
